...
ls -la
sudo chmod 777 ./data.zip

sudo unzip data.zip
sudo chmod ./ModelNet40_MeshNet
sudo rm -rf data.zip
sudo rm -rf ./ModelNet40_MeshNet/.DS_Store

sudo wget --load-cookies /tmp/cookies.txt "https://docs.google.com/uc?export=download&confirm=$(wget --quiet --save-cookies /tmp/cookies.txt --keep-session-cookies --no-check-certificate 'https://docs.google.com/uc?export=download&id=1l8Ij9BODxcD1goePBskPkBcgKW76Ewcs' -O- | sed -rn 's/.*confirm=([0-9A-Za-z_]+).*/\1\n/p')&id=1l8Ij9BODxcD1goePBskPkBcgKW76Ewcs" -O MeshNet_best_9192.pkl && rm -rf /tmp/cookies.txt
